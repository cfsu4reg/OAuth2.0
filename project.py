...
def gconnect():
    ...
    if request.args.get('state') != login_session['state']:
        ...
        response.headers['Contest-Type'] = 'applicatioin/json'
        return response
    code = request.data
    try:
    ...
    if result.get('error') is not None:
        resonse = make_response(json.dumps(result.get('error')), 501)
        response.headers['Content-Type'] = 'application/json'
